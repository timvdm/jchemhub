...
goog.require('kemia.util.BondUtil');
goog.require('kemia.util.BondUtil.Orders');
...
kemia.io.smiles.SmilesParser = function(){
	alert("kemia.smiles.SmilesParser = function()")
}

/**
 * Parse a SMILES String into a kemia.model.Molecule.
 */
kemia.io.smiles.SmilesParser.parse = function(smiles){

    //Initialization before parse 
    var bond = new kemia.model.Bond;
    var nodeCounter = 0;
    var bondOrder = null;  
    var bondOrderForRingClosure = kemia.util.BondUtil.Orders.SINGLE;
	...
	do {
        ...
        if ((mychar >= 'A' && mychar <= 'Z') || (mychar >= 'a' && mychar <= 'z') ||(mychar == '*'))
        {
            ...
            if (mychar == '*') {
                currentSymbol = "*";
                atom = new kemia.model.PseudoAtom("*",0,0);
            }
			else {
				...
				if (currentSymbol != null) {
                    if (currentSymbol.length == 1) {
                        if (currentSymbol.toUpperCase() != currentSymbol ) { 
                            ...
                            atom = new kemia.model.Atom(currentSymbol);
            ...
            if ((lastNode != null) && bondExists) {
				bond = kemia.util.BondUtil.getBond (atom, lastNode,bondOrder);
                if (bondIsAromatic) {
                    bond.aromatic=true;
                }
                molecule.addBond(bond);
            }
            bondOrder = kemia.util.BondUtil.Orders.SINGLE;
            lastNode = atom;
            nodeCounter++;
            ...
            if (status == 2 || !((smiles.charAt(position) >= '0' && smiles.charAt(position) <= '9') || smiles.charAt(position) == '%'))
                bondOrder = kemia.util.BondUtil.Orders.DOUBLE;
            else
                bondOrderForRingClosure = kemia.util.BondUtil.Orders.DOUBLE;
        } 
    	else if (mychar == '#') {
            //alert(mychar);
            position++;
            if (status == 2 || !((smiles.charAt(position) >= '0' && smiles.charAt(position) <= '9') || smiles.charAt(position) == '%')) 
                bondOrder = kemia.util.BondUtil.Orders.TRIPLE;
            else 
                bondOrderForRingClosure = kemia.util.BondUtil.Orders.TRIPLE;
        } 
		else if (mychar == '(') {
            //alert(mychar);
			...
			if (ringNode != null) {
			    ...
			    ringBond = kemia.util.BondUtil.getBond(lastNode, ringPartner, bondOrd);
			    if (bondIsAromatic) {
			...
			bondOrderForRingClosure = kemia.util.BondUtil.Orders.SINGLE;
 ...
 *
 */
kemia.io.smiles.SmilesParser.getSymbolForOrganicSubsetElement = function(s, pos){
    var possibleSymbol="";
	if (pos < s.length - 1){
...
kemia.io.smiles.SmilesParser.getAtomString = function(smiles, pos) {
    atomString = "";
...
kemia.io.smiles.SmilesParser.getElementSymbol = function(s,pos) {
...
kemia.io.smiles.SmilesParser.getImplicitHydrogenCount = function(s,position) {
    count = 0;  
...
kemia.io.smiles.SmilesParser.assembleAtom = function(s) {
    atom = null;
    position = 0;
    currentSymbol = null;
    isotopicNumber = "";
